...
export const PostUploadAnalysis: React.FC<Props> = ({ data, schema, aiResults, onClose }) => {
  ...
  // 시각화 축 매핑 (정규화된 필드 우선 사용)
  const chartData = data.slice(0, 20).map(d => {
    // AI가 지정한 xAxisKey가 없어도 표준 'region' 필드 활용
    const label = d.region || d.title || '미분류';
    // 수치형 필드 중 유효한 값 선택
    const value = Number(d.resident_count || d.budget || 0);
    
    return {
      name: String(label).substring(0, 8),
      value: value
    };
  }).filter(d => d.value > 0);
    ...
    <div className="animate-fadeIn space-y-8 pb-20">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div>
            <div className="flex items-center space-x-2 mb-1">
              <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-md uppercase tracking-tighter">Verified Dataset</span>
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">{schema.datasetName}</h2>
            </div>
            <p className="text-slate-500 font-medium text-sm">표준 스키마 정규화가 완료되었습니다. ({data.length} records)</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-slate-200 shadow-xl overflow-hidden relative">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-xl font-black text-slate-800 flex items-center">
              <i className="fa-solid fa-chart-area mr-3 text-indigo-500"></i>
        ...
        <div className="space-y-6">
          <h3 className="text-lg font-black text-slate-800 flex items-center px-2 underline decoration-indigo-200 underline-offset-4">
            AI 자동 인사이트
          </h3>
          <div className="space-y-4">
            {aiResults.insights.map((insight, idx) => (
              <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all border-l-4 border-l-indigo-500">
      ...
      <div className="bg-slate-900 rounded-[3.5rem] p-12 text-white shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 blur-[120px]"></div>
        <h3 className="text-3xl font-black mb-8 tracking-tighter">심층 분석 시나리오</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {aiResults.recommendations.map((rec) => (
            <button key={rec.id} onClick={() => handleDeepDive(rec)} className="text-left p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-indigo-500/50 hover:bg-white/10 transition-all group">
              <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center mb-6 shadow-lg group-hover:scale-110 transition-transform">
                <i className="fa-solid fa-bolt text-white"></i>
              </div>
              <h4 className="font-black text-lg mb-2 group-hover:text-indigo-400">{rec.title}</h4>
          ...
          <div className="bg-white/5 rounded-[2.5rem] p-10 border border-white/10 animate-fadeIn">
            {isAnalyzing ? (
              <div className="text-center py-10">
                ...
                <p className="font-black text-xl">데이터 사이언스 리포트 생성 중...</p>
              </div>
            ) : deepDive && (
              <div className="space-y-8 max-w-4xl mx-auto">
                <div className="flex justify-between items-center border-b border-white/10 pb-6">
                  <h3 className="text-2xl font-black text-indigo-400">{deepDive.reportTitle}</h3>
                  <span className="bg-amber-500/20 text-amber-400 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-amber-500/30">
                    Risk Assessment: {deepDive.riskFactor}
                  </span>
                </div>
