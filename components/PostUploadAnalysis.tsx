...
export const PostUploadAnalysis: React.FC<Props> = ({ data, schema, aiResults, onClose }) => {
  ...
  const chartData = data.slice(0, 20).map(d => {
    const rawLabel = regionMapping ? d[regionMapping.target] : 'Item';
    const label = rawLabel ? String(rawLabel) : 'Unknown';
    const value = valueMapping ? Number(d[valueMapping.target]) : 0;
    
    return {
      name: label.substring(0, 10),
      value: value
    };
  }).filter(d => d.value > 0);
  ...
  return (
    <div className="animate-fadeIn space-y-8 pb-20">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div>
            <div className="flex items-center space-x-2 mb-1">
              ...
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">{String(schema.datasetName || '분석 리포트')}</h2>
            </div>
            <p className="text-slate-500 font-medium text-sm">
              물리 테이블 <code className="bg-slate-100 px-2 py-1 rounded text-emerald-600 font-bold">{String(schema.tableName)}</code>에 데이터 적재 완료
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-slate-200 shadow-xl overflow-hidden relative">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-xl font-black text-slate-800 flex items-center">
              ...
              {String(schema.yAxisLabel || '통계')} 분석
            </h3>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Tooltip 
                   ...
                   formatter={(value) => [new Intl.NumberFormat('ko-KR').format(Number(value)) + (schema.unit || ''), String(schema.yAxisLabel || '')]}
                />
          ...
          <div className="space-y-4">
            {aiResults.insights.map((insight, idx) => (
              <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all border-l-4 border-l-emerald-500">
                <h4 className="font-black text-slate-800 text-sm mb-1">{String(insight.title)}</h4>
                <p className="text-slate-500 text-xs leading-relaxed font-medium">{String(insight.content)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-slate-900 rounded-[3.5rem] p-12 text-white shadow-2xl relative overflow-hidden">
        ...
        {errorMessage && <p className="text-red-400 mb-4 font-bold text-sm">⚠️ {errorMessage}</p>}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {aiResults.recommendations.map((rec) => (
            <button key={rec.id} onClick={() => handleDeepDive(rec)} className="text-left p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-emerald-500/50 hover:bg-white/10 transition-all group">
              ...
              <h4 className="font-black text-lg mb-2 group-hover:text-emerald-400">{String(rec.title)}</h4>
              <p className="text-slate-400 text-xs font-medium leading-relaxed">{String(rec.description)}</p>
            </button>
          ))}
        </div>
            ...
            {isAnalyzing ? (
              ...
              <div className="space-y-8 max-w-4xl mx-auto">
                <div className="flex justify-between items-center border-b border-white/10 pb-6">
                  <h3 className="text-2xl font-black text-emerald-400">{String(deepDive.reportTitle)}</h3>
                ...
                <p className="text-slate-300 text-lg leading-relaxed font-medium italic">"{String(deepDive.summary)}"</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {deepDive.strategicSuggestions.map((s, i) => (
                    <div key={i} className="bg-white/5 p-5 rounded-2xl flex items-center space-x-4 border border-white/5">
                      ...
                      <span className="text-sm font-bold text-slate-200">{String(s)}</span>
                    </div>
                  ))}
                </div>
              </div>
